...
class HandsetWorkflows(lib.testcasebase.Tele2Test):
    def test_handset_postpaid_noporting_delivery(self, c_c, workflow='handset',
    ...
    def test_handset_postpaid_porting_delivery(self, c_c, workflow='handset', profile='handset_postpaid_porting_delivery'):
        self.go_to_step4(workflow, profile)
        self.elementcheck('step_4', 'lastpage')
        self.get_screenshot('succesfull', 'test_handset_postpaid_porting_delivery')

    def test_handset_postpaid_noporting_clickandcollect(self, c_c, workflow='handset',
                                                        c_c= True,
    ...
    def test_handset_postpaid_porting_clickandcollect(self, c_c, workflow='handset',
                                                      c_c= True,
...
class PrepaidWorkflows(lib.testcasebase.Tele2Test):
    def test_simonly_prepaid(self, c_c, workflow='simonly_prepaid', profile='simonly_prepaid'):
        self.go_to_step3_prepaid(workflow, profile)
        self.elementcheck('step_4', 'lastpage')
        self.get_screenshot('succesfull', 'test_simonly_prepaid')

    def test_handset_prepaid(self, c_c, workflow='handset_prepaid', profile='handset_prepaid'):
        self.go_to_step3_prepaid(workflow, profile)
        self.elementcheck('step_4', 'lastpage')
        self.get_screenshot('succesfull', 'test_simonly_prepaid')
...
class SimOnlyFieldCorrection(lib.testcasebase.Tele2Test):
    def test_firstletter_field_correction(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow, c_c)
        self.field_validation('step_1', 'input_initials', 'input_firstname', settings.INITIALS, 'corrected',
                              'check_initials')

    def test_phonenumber_field_correction(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow, c_c)
        self.field_validation('step_1', 'input_phonenumber', 'input_firstname', settings.PHONENUMBER, 'corrected',
                              'check_phonenumber')

    def test_housenumber_field_correction(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow, c_c)
...
class SimOnlyFieldMandatory(lib.testcasebase.Tele2Test):
    def test_gender_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'gender')

    def test_firstname_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'firstname')

    def test_lastname_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'lastname')

    def test_initials_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'initials')

    def test_DOB_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'DOB')

    def test_postcode_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'postcode')

    def test_housenumber_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'housenumber')

    def test_streetname_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow, c_c)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'streetname')

    def test_city_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow, c_c)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'city')

    def test_phonenumber_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow, c_c)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'phonenumber')

    def test_email_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow, c_c)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'e-mail')

    def test_repeat_email_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step1(workflow, c_c)
        self.elementcheck('step_1', 'button_next_step', click=True)
        self.errorcheck('step_1', 'repeat_email')

    def test_IBAN_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step2(workflow)
        self.elementcheck('step_2', 'button_next_step', click=True)
        self.errorcheck('step_2', 'IBANnumber')

    def test_documenttype_mandatory(self, c_c, workflow='sim_only'):
        self.go_to_step2(workflow)
        self.elementcheck('step_2', 'button_next_step', click=True)
        self.errorcheck('step_2', 'documenttype')

    def test_documentnumber_mandatory(self, c_c, workflow='sim_only', profile='default'):
        self.go_to_step2(workflow)
    ...
    def test_current_subscription_mandatory(self, c_c, workflow='sim_only', profile='porting_mandatory'):
        self.go_to_step2(workflow)
        self.dropdownselector(profile, 'step_2', 'select_porting', 'porting', 'porting')
        self.elementcheck('step_2', 'button_next_step', click=True)
        self.errorcheck('step_2', 'current_subscription')

    def test_current_phonenumber_mandatory(self, c_c, workflow='sim_only', profile='porting_mandatory'):
        self.go_to_step2(workflow)
        self.dropdownselector(profile, 'step_2', 'select_porting', 'porting', 'porting')
        self.elementcheck('step_2', 'button_next_step', click=True)
        self.errorcheck('step_2', 'current_phonenumber')

    def test_current_mobile_provider_mandatory(self, c_c, workflow='sim_only', profile='porting_mandatory'):
        self.go_to_step2(workflow)
        self.dropdownselector(profile, 'step_2', 'select_porting', 'porting', 'porting')
        self.elementcheck('step_2', 'button_next_step', click=True)
        self.errorcheck('step_2', 'mobile_provider')

    def test_current_simcard_number_mandatory(self, c_c, workflow='sim_only', profile='porting_mandatory'):
        self.go_to_step2(workflow)
        self.dropdownselector(profile, 'step_2', 'select_porting', 'porting', 'porting')
        self.elementcheck('step_2', 'button_next_step', click=True)
        self.errorcheck('step_2', 'simcard_number')

    def test_current_portingdate_mandatory(self, c_c, workflow='sim_only', profile='porting_mandatory'):
        self.go_to_step2(workflow)
        self.dropdownselector(profile, 'step_2', 'select_porting', 'porting', 'porting')
        self.elementcheck('step_2', 'button_next_step', click=True)
...
class SimOnlyWorkflows(lib.testcasebase.Tele2Test):
    def test_simonly_postpaid_noporting_delivery(self, c_c, workflow='sim_only',
                                                 profile='simonly_postpaid_noporting_delivery',):
        self.go_to_step4(workflow, profile)
        self.elementcheck('step_4', 'lastpage')
        self.get_screenshot('succesfull', 'test_simonly_postpaid_noporting_delivery')

    def test_simonly_postpaid_porting_delivery(self, c_c, workflow='sim_only', profile='simonly_postpaid_porting_delivery'):
        self.go_to_step4(workflow, profile)
        self.elementcheck('step_4', 'lastpage')
        self.get_screenshot('succesfull', 'test_simonly_postpaid_porting_delivery')

    def test_simonly_postpaid_noporting_clickandcollect(self, c_c, workflow='sim_only',
    ...
    def test_simonly_postpaid_porting_clickandcollect(self, c_c, workflow='sim_only',
...
class SimOnlyShoppingcart(lib.testcasebase.Tele2Test):
    def test_configurepage_cart(self, c_c, workflow='sim_only'):
        self.go_to_configpage(workflow)
        self.shoppingcart_configpage()

    def test_step1_cart(self, c_c, workflow='sim_only', profile='default'):
